...
export default function Stickers({user, isMobile}: {user: any, isMobile: boolean }) {
    ...
    const [inventoryCopied, setInventoryCopied] = useState<boolean>(false);
    ...
    useEffect(() => {
        ...
        //TODO:
        //convert stickers inventory to text
        ...
        for (const [albumEnum, albumString] of Object.entries(Album)) {
            counter += 1;
            const extraTemp = extra.filter((sticker: Sticker) => sticker.album === (Album as any)[albumEnum] && sticker.tradeable);
            console.log("extra: ", extraTemp);
            const needTemp = missing.filter((sticker: Sticker) => sticker.album === (Album as any)[albumEnum] && sticker.tradeable);
            console.log("missing: ", needTemp);
        
            let temp: string = `-Set ${counter} (${albumString})-\n`;
            temp += `HAVE: [${extraTemp.map((sticker: Sticker) => `${sticker.name}${"⭐".repeat(sticker.star)}`).join(", ")}]\n`;
            temp += `WANT: [${needTemp.map((sticker: Sticker) => `${sticker.name}${"⭐".repeat(sticker.star)}`).join(", ")}]\n\n`;
        
            transformedText += temp;
        }

        transformedText += "Template generated by mgo-trading.com";

        setInventoryText(transformedText);
    ...
    <div className="mx-1 sm:mx-0">
        ...
        <div className="sm:flex sm:justify-between sm:mx-2 lg:mx-6">
            <Grid container spacing={1}>
                <Grid item xs={12} sm={11} md={10} className="flex items-center justify-center ml-2 border border-white rounded-lg border-2 p-4 mt-6 sm:mt-0">
                    ...
                    <Button
                        ...
                        className='text-white ml-4 border border-opacity-100 border-white bg-teal-500 p-4'
                        variant='outlined'
                        startIcon={<FileCopyIcon />}
                        onClick={handleInventoryCopy}
                    >
                        {inventoryCopied ? 'Copied!' : 'Copy'}
                    </Button>
                    </div>
                </Grid>
